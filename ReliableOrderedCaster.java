...
public class ReliableOrderedCaster extends Multicaster {
	...
	private ArrayList<MyMessage> CausaldeliveredList = new ArrayList<MyMessage>();
	...
	public void basicreceive(int peer,Message message) {
		...
		if (id == sequencerId){
			...
			if (localSequence != receivedMessage.sequence){
				...
				// Causal order logic Start
				for ( MyMessage n : historyList ){
					if (!CausaldeliveredList.contains(n)){
				     		mcui.deliver(n.getSender(), n.text);
						CausaldeliveredList.add(n);
						historyList.add(n);
				    }
				}
				// Causal order logic End
				...
				deliveredList.add(receivedMessage);
				CausaldeliveredList.add(receivedMessage); // Keeping track of Causal delivery
